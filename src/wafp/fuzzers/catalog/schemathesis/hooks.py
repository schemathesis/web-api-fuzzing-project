...
The implementation is designed in the way that the original Schemathesis code requires minimal changes.
"""
...
def disable_format_strategies() -> None:
    ...
    from hypothesis_jsonschema._from_schema import STRING_FORMATS as H_STRING_FORMATS
    from schemathesis.specs.openapi._hypothesis import STRING_FORMATS

    STRING_FORMATS.clear()
    H_STRING_FORMATS.clear()
...
def use_less_schema_pre_processing() -> None:
    ...
    def from_schema(schema: Union[dict, bool]) -> st.SearchStrategy[JSONType]:
        ...
        if schema is True or schema == {}:
            return JSON_STRATEGY  # type: ignore
        # Otherwise, we're dealing with "objects", i.e. dicts.
        if not isinstance(schema, dict):
            raise InvalidArgument(f"Got schema={schema} of type {type(schema).__name__}, " "but expected a dict.")
        ...
        if "anyOf" in schema:
            tmp = schema.copy()
            return st.one_of([from_schema({**tmp, **canonicalish(s)}) for s in tmp.pop("anyOf")])
        if "allOf" in schema:
            tmp = schema.copy()
        ...
        if "oneOf" in schema:
            ...
            if len(schemas) > len({encode_canonical_json(s) for s in schemas}):
                return st.nothing()
            return st.one_of([from_schema(s) for s in schemas]).filter(
                lambda inst: sum(is_valid(inst, s) == 1 for s in schemas)
            )
        ...
        if multiple_of is not None or "integer" in schema["type"]:
            ...
            if multiple_of is not None:
                ...
                if upper is not None:
                    ...
                    upper //= multiple_of
                return st.integers(lower, upper).map(lambda x: x * multiple_of)  # type: ignore
            return st.integers(lower, upper)
        strategy = st.floats(min_value=lower, max_value=upper, allow_nan=False, allow_infinity=False)
        if schema.get("exclusiveMaximum") is not None or schema.get("exclusiveMinimum") is not None:
    ...
    def rfc3339(name: str) -> st.SearchStrategy[str]:
        """Given the name of an RFC3339 date or time format, return a strategy for conforming values.
        ...
        if name == "time-numoffset":
            return st.tuples(st.sampled_from(["+", "-"]), rfc3339("time-hour"), rfc3339("time-minute")).map(":".join)
        if name == "time-offset":
        ...
        if "pattern" in schema:
            ...
            strategies = {
                ...
                **{name: rfc3339(name) for name in RFC3339_FORMATS},
                ...
                "ipv4": st.ip_addresses(v=4).map(str),  # type: ignore
                "ipv6": st.ip_addresses(v=6).map(str),  # type: ignore
                **{name: domains.map("https://{}".format) for name in url_synonyms},
                "json-pointer": st.just(""),
                "relative-json-pointer": st.just(""),
                "regex": REGEX_PATTERNS,
            }
            if schema["format"] in strategies:
                strategy = strategies[schema["format"]]
    ...
    def array_schema(schema: dict) -> st.SearchStrategy[List[JSONType]]:
        ...
        if isinstance(items, list):
            ...
            if contains is not None:
                assert additional_items == {}, "Cannot handle additionalItems and contains togther"
                additional_items = contains
                min_size = max(min_size, 1)
            fixed_items = st.tuples(*map(from_schema, items))
            extra_items = st.lists(from_schema(additional_items), min_size=min_size, max_size=max_size)
            return st.tuples(fixed_items, extra_items).map(lambda t: list(t[0]) + t[1])  # type: ignore
        if contains is not None:
    ...
    def object_schema(schema: dict) -> st.SearchStrategy[Dict[str, JSONType]]:
        ...
        # pylint: disable=too-many-branches,too-many-nested-blocks,redefined-outer-name
        ...
        max_size = min(
            ...
            if ("propertyNames" in schema or "properties" in schema) and not additional_allowed
            else float("inf"),
        )
        ...
        all_names_strategy = st.one_of(
            st.sampled_from(sorted(dep_names) + sorted(dep_schemas)) if (dep_names or dep_schemas) else st.nothing(),
            from_schema(names),
                ...
                if key in properties:
                    ...
                    for rgx, matching_schema in patterns.items():
                        if re.search(rgx, string=key) is not None:
                            ...
                            for rgx, matching_schema in patterns.items():
                                if re.search(rgx, string=key) is not None and not is_valid(out[key], matching_schema):
                                    out.pop(key)
                                    elements.reject()
                                    break
                            break
    ...
    def from_schema_patched(
        schema: dict, *, custom_formats: Dict[str, st.SearchStrategy[str]]
    ) -> st.SearchStrategy[JSONType]:
        return from_schema(schema)

    patched = patch("schemathesis.specs.openapi._hypothesis.from_schema", from_schema_patched)
    patched.start()

    patched = patch("schemathesis.specs.openapi.negative.from_schema", from_schema_patched)
    patched.start()
