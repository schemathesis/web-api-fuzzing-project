...
def test_compose_up(target):
    output = target.compose.up()
    assert b"wafp_example_web_1 ... done" in output


def test_compose_up_build(target):
    output = target.compose.up(build=True)
...
def test_compose_logs(target):
    assert_until(lambda: b"Uvicorn running on" in target.compose.logs())


@pytest.mark.usefixtures("running_target")
def test_compose_stop(target):
    assert b"Stopping wafp_example_web_1 ... done" in target.compose.stop()


@pytest.mark.usefixtures("running_target")
def test_compose_rm(target):
    assert b"Removing wafp_example_web_1 ... done" in target.compose.rm()
...
def test_manually_removed_image(target):
    ...
    assert_until(lambda: b"Uvicorn running on http://0.0.0.0:80 (Press CTRL+C to quit)\n" in target.compose.logs())
